...
def executar_joc(partida:Game):
    win, numRound = partida.user_game()
    return win, numRound

def resultat_no_anonymous(partida:Game, win:bool, numRound:int, player:Player):
    mostrar_guanyar(win)
    if (win == True):
...
def resultat_anonymous(partida:Game, win:bool, numRound:int):
    if (win == True):
